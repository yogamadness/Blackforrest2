...
if(empty($halaman)){
	...
	$s=mysql_query("SELECT * FROM invitation where id_category not in(25,24,31) ORDER BY id_product DESC LIMIT $posisi,$batas");
	if($s === FALSE){
		die(mysql_error());
	...
	//Paging 
	$query = mysql_query("SELECT * FROM invitation where id_category not in(25,24,31)");
	...
	for ($i=1; $i<=$total_pages; $i++) { 
            echo "<li class='p$i page'><a href='invitations.php?page=all&halaman=".$i."'>".$i."</a></li> "; 
	}; 
