...
if(empty($halaman)){
	...
	$s=mysql_query("SELECT * FROM invitation where id_category = 27 ORDER BY id_product DESC LIMIT $posisi,$batas");
	if($s === FALSE){
		die(mysql_error());
	...
	//Paging 
	$query = mysql_query("SELECT * FROM invitation where id_category = 27");
	...
	for ($i=1; $i<=$total_pages; $i++) { 
            echo "<li class='p$i page'><a href='invitations.php?page=fam&halaman=".$i."'>".$i."</a></li> "; 
	}; 
